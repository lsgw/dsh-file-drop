...
- `.dsh-drops`、staging、backup 和目标路径中的 symlink/junction 会被拒绝。新建目录/文件分别请求私有 `0700/0600` 模式；clear 先取消活动会话，再原子隔离并后台删除，返回 `cleanupPending` / `cleanupError`；重启遗留 quarantine 仍计入 size 与累计配额。
...
- metadata 阶段只使用 Node `fs/promises` 在当前和其他活动 Session 的 Host 签发工作区根中做有界递归扫描；最多访问 20000 个条目，硬预算为 10 秒。
...
- 搜索范围只来自 Host 根据 Session 生成的可信工作区根，不扩展到固定用户目录、系统文件索引或平台专属搜索根。
